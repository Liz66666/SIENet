...
class AnchorHeadTemplate(nn.Module):
    ...
    def get_cls_layer_loss(self):
        cls_preds = self.forward_ret_dict['cls_preds'] # [B, H, W, 6]
        box_cls_labels = self.forward_ret_dict['box_cls_labels'] # [B, H*W*6]
        batch_size = int(cls_preds.shape[0])
        cared = box_cls_labels >= 0  # [B, H*W*6]
        positives = box_cls_labels > 0
        negatives = box_cls_labels == 0
        negative_cls_weights = negatives * 1.0
        cls_weights = (negative_cls_weights + 1.0 * positives).float() # [B, H*W*6]
        ...
        cls_targets = cls_targets.unsqueeze(dim=-1) # [B, H*W*6, 1]

        cls_targets = cls_targets.squeeze(dim=-1)# [B, H*W*6]
        one_hot_targets = torch.zeros(
            *list(cls_targets.shape), self.num_class + 1, dtype=cls_preds.dtype, device=cls_targets.device
        )# [B, H*W*6, 2]
        one_hot_targets.scatter_(-1, cls_targets.unsqueeze(dim=-1).long(), 1.0)
        cls_preds = cls_preds.view(batch_size, -1, self.num_class) # [B, H*W*6, 1]
        one_hot_targets = one_hot_targets[..., 1:]
        # import pdb
        # pdb.set_trace()
        cls_loss_src = self.cls_loss_func(cls_preds, one_hot_targets, weights=cls_weights)  # [B, H*W*6, 1]
